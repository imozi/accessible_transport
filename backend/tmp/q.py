...
async def fetch_data():
    conn = await asyncpg.connect(user='postgres', password='12345', database='accessible_transport', host='localhost',
                                 port=5432)

    schedules = await conn.fetch('SELECT id, work_day, work_time FROM employee')
    requests = await conn.fetch('SELECT id, date, time_start, time_end FROM request')
...
async def distribute_requests():
    schedules, requests = await fetch_data()

    distribution = {employee['id']: [] for employee in schedules}
    lunch_breaks = {}
    ...
    for employee in schedules:
        ...
        if min_lunch_start_dt < max_lunch_end_dt:
            ...
            lunch_breaks[employee['id']] = (lunch_start, lunch_end)
            distribution[employee['id']].append({
                'id': 'Обед',
    ...
    for request in requests:
        ...
        for employee in schedules:
            ...
            if is_within_schedule(request_start, request_end, request_date, schedule_start, schedule_end, work_day):
                lunch_start, lunch_end = lunch_breaks.get(employee['id'], (None, None))
                ...
                if lunch_start and not (
                    ...
                    if all(time_with_timedelta(r['time_end'], timedelta(minutes=20)) <= request_start or
                           ...
                           for r in distribution[employee['id']]):
        ...
        if available_employees:
            ...
            distribution[employee_id].append({
                'id': request['id'],
                'date': request_date,
    ...
    return distribution, unassigned_requests


async def main():
    distribution, unassigned_requests = await distribute_requests()

    for employee_id, assigned_requests in distribution.items():
        ...
        for request in assigned_requests_sorted:
            if request['id'] == 'Обед':
                ...
                print(
                    f"  Заявка ID {request['id']}, дата {request['date']} с "
                    f"{request['time_start']} до {request['time_end']}")
