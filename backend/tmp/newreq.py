...
async def distribute_requests():
    # Получение всех сотрудников и заявок
    schedules = Employee.objects.all()
    requests = Request.objects.filter(status=RequestStatus.objects.get(name='Новая'))
    ...
    for request in requests:
        ...
        for employee in schedules:
            ...
            if is_within_schedule(request_start, request_end, request_date, schedule_start, schedule_end, work_day):
                ...
                if lunch_start and not (
                    ...
                    # Проверка на пересечение с другими заявками и промежуток в 15 минут
        ...
        if available_employees:
            ...
            # Назначение сотрудника и изменение статуса заявки
            request.employee = employee
            request.status = RequestStatus.objects.get(name='Назначена')
            request.save()
        else:
...
asyncio.run(distribute_requests())
