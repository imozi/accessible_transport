...
const formSchema = toTypedSchema(
  object({
    ...
    is_pacemaker: string().default("false").transform((value) => Boolean(value)),
    description: string().trim().optional().transform((value) => {
      if (value) {
        return value
      } else {
        return null
  ...
  <Modal :props="modal.props" v-model:open="open">
    ...
    category.code }}
        ...
        <UiFormField v-slot="{ componentField }" name="is_pacemaker">
          <UiFormItem class="passanger-form__item">
            ...
            <UiSelect v-bind="componentField">
              ...
              <UiSelectContent>
                <UiSelectGroup>
                  <UiSelectItem value="false">Нет
                  </UiSelectItem>

                  <UiSelectItem value="true">Да
                  </UiSelectItem>
                </UiSelectGroup>
              </UiSelectContent>
            </UiSelect>
      ...
      <UiDialogFooter class="mb-5">
        <UiDialogClose class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border bg-background hover:bg-accent h-10 px-4 py-2 text-[#3A76EE] hover:text-[#3A76EE] border-[#3976EE]" type="button">
